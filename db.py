...
def show_customer_orders():
    ...
    order_win.geometry("800x500")

    # Treeview to display orders
    columns = ["Order ID", "Date", "Status", "Total", "Items", "Actions"]
    tree = ttk.Treeview(order_win, columns=columns, show="headings")
    ...
    cursor.execute("""
        SELECT o.Order_ID, o.Order_Time, o.Status, p.Amount as Total_payment
        FROM Orders o
    ...
    for order in cursor.fetchall():
        # Get order items
        cursor.execute("""
            SELECT mi.Name, c.Quantity 
            FROM contains c
            JOIN Menu_Item mi ON c.Item_ID = mi.Item_ID
            WHERE c.Order_ID = %s
        """, (order['Order_ID'],))
        items = ", ".join([f"{item['Name']} (x{item['Quantity']})" for item in cursor.fetchall()])

        status = order['Status']
        cancel_btn = "Cancel" if status in ["Pending", "Processing"] else ""
        tree.insert("", "end", values=(
            order['Order_ID'],
            order['Order_Time'].strftime("%Y-%m-%d %H:%M"),
            status,
            f"${order['Total_payment']:.2f}",
            items,
            cancel_btn
        ))
    ...
    def on_cancel():
        ...
        if selected:
            ...
            cancel_order(conn, cursor, order_id)
            order_win.destroy()
            show_customer_orders()

    cancel_btn = ttk.Button(order_win, text="Cancel Selected Order", command=on_cancel)
    ...
    order_win.protocol("WM_DELETE_WINDOW", on_close)

def cancel_order(conn, cursor, order_id):
    try:
        ...
        cursor.execute("""
            INSERT INTO Refund (Payment_ID, Amount, Status_s, Requested_At)
            SELECT Payment_ID, Amount, 'Pending', NOW()
            FROM Payment
            WHERE Order_ID = %s
        """, (order_id,))
        ...
        cursor.execute("""
            ...
            WHERE Employee_ID IN (
                SELECT D_Employee_ID FROM Delivery_Order
                WHERE Order_ID = %s
            )
        """, (order_id,))

        conn.commit()
        messagebox.showinfo("Success", "Order cancelled successfully. Refund initiated.")
    except Exception as e:
        conn.rollback()
...
def build_orders_tab(frame):
    ...
    def calculate_order_total(order_id):
        try:
            ...
            cursor.execute("""
                SELECT SUM(mi.Price * c.Quantity) as total
                FROM contains c
                JOIN Menu_Item mi ON c.Item_ID = mi.Item_ID
                WHERE c.Order_ID = %s
            """, (order_id,))
            ...
            cursor.execute("""
                ...
                WHERE Order_ID = %s
            """, (order_id,))
            delivery_fee = cursor.fetchone()
            if delivery_fee:
                total += float(delivery_fee[0])
    ...
    def save_order():
        try:
            ...
            if cursor.fetchone():
                ...
                # New order - insert without payment
                cursor.execute("""
                ...
                messagebox.showinfo("Success", "New order created successfully. Add items to calculate total.")
    ...
    def delete_order():
        ...
        try:
            ...
            cursor.execute("DELETE FROM Payment WHERE Order_ID=%s", (int(order_id),))
    ...
    def add_item_to_order():
        ...
        try:
            ...
            cursor.execute("SELECT Item_ID, Price FROM Menu_Item WHERE Item_ID=%s", (int(item_id),))
            ...
            # Add/update the item
            cursor.execute("""
                INSERT INTO contains (Order_ID, Item_ID, Quantity)
                VALUES (%s, %s, %s)
                ON DUPLICATE KEY UPDATE Quantity = VALUES(Quantity)
            """, (int(order_id), int(item_id), int(quantity)))

            # Calculate total amount from all order items
            total_amount = calculate_order_total(int(order_id))

            # Check if payment exists
            cursor.execute("SELECT Payment_ID FROM Payment WHERE Order_ID=%s", (int(order_id),))
            ...
            if payment_exists:
                ...
                cursor.execute("""
                    ...
                    SET Amount = %s 
                    WHERE Order_ID = %s
                """, (total_amount, int(order_id)))
            else:
                # Create new payment
                payment_id = int(order_id)  # or generate a unique ID
                payment_method = "OL"  # Default to online payment
                cursor.execute("""
                    INSERT INTO Payment (Payment_ID, Order_ID, Payment_Method, Amount, Payment_Date)
                    VALUES (%s, %s, %s, %s, NOW())
                """, (payment_id, int(order_id), payment_method, total_amount))

            conn.commit()
            messagebox.showinfo("Success", "Item added to order and payment updated.")
